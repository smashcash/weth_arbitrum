...
async function generateMerkleProof(deposit) {
  ...
  const events = await smashnado.getPastEvents('Deposit', { fromBlock: 	0, toBlock: 	'latest' })
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 42161) {
        console.log('Transaction submitted through the relay.')
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e7 })
      .on('transactionHash', function (txHash) {
        if (netId === 42161) {
          console.log(`View transaction on Etherscan https://etherscan.io/tx/${txHash}`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'weth', amount = '0.1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/ETHSmashnado.json')).json()
  ...
  if (!(netId === 42161)) {
    throw new Error('You did not connect to Arbitrum network. Please make sure you connected to Arbitrum Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Arbitrum Mainnet.')
  erc20 = currency == 'weth' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'weth', amount: '0.1' }
  //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient WETH Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account ETH balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
